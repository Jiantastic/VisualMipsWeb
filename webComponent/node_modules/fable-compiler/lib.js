...
function runCommand(workingDir, command) {
    return new Promise(function (resolve, reject) {
        runCommandPrivate(workingDir, command, { resolve: resolve, reject: reject });
    });
}
...
function getCommonBaseDir(filePaths) {
    ...
    var normalized = filePaths.map(function (filePath) { return path.dirname(filePath).replace(/\\/g, '/').split('/'); });
...
function babelify(babelAst, opts) {
    ...
    if (opts.sourceMaps && babelAst.originalFileName) {
        try {
            ...
            babelOpts.sourceMaps = opts.sourceMaps,
                ...
                babelOpts.sourceFileName = path.relative(path.dirname(babelAst.fileName), babelAst.originalFileName.replace(/\\/g, '/'));
        }
        catch (err) {
            // Do nothing
        }
    ...
    if (Array.isArray(babelAst.jsIncludes))
        babelAst.jsIncludes.forEach(function (js) {
...
function babelifyToFile(babelAst, opts) {
    ...
    babelify(babelAst, opts).forEach(function (parsed) {
        return writeFile(parsed.fileName, parsed.code, opts.sourceMaps === true ? parsed.map : null);
    });
}
