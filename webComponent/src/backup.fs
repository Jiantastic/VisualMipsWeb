...
module main = 
    ...
    let z = "ADD 1,2,3      # this is a comment!"
    ...
    *)
    ...
    //             | Word m -> errorLog.insertAdjacentHTML("beforeend",string(m) + " ")
                

    //     errorLog.insertAdjacentHTML ("beforeend","<p>Line " + string(currentLineNumber) + ": blablabla " + q ourMachineState + " " + "</p>")
    //     registerStateToString ourMachineState
    ...
    let executeHandler() = 
        ...
        let eachLineProcessing (currentLine : int) (currentMachineState : MachineState) =
            ...
            let output (mach : MachineState) = mach
        ...
        // will work for step forward, need to modify this slightly to get it working for step backward
        let rec processAllCodeMirrorInput (mach : MachineState) (startLine : int) (lastLine : int)  = if startLine=lastLine then eachLineProcessing lastLine mach else processAllCodeMirrorInput (eachLineProcessing startLine mach) (startLine+1) lastLine 
        processAllCodeMirrorInput initialise 0 (cmEditor.lastLine()) 
